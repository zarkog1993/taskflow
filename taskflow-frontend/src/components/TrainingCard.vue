  ...
  <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/80 rounded-xl p-4 shadow-sm hover:border-indigo-500/50 transition">
      ...
      <div>
        ...
        <span
          :class="[
            ...
            session.type === 'match' ? 'bg-emerald-500/15 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-400 border border-emerald-500/30' :
            session.type === 'tactical_analysis' ? 'bg-purple-500/15 text-purple-600 dark:bg-purple-500/20 dark:text-purple-400 border border-purple-500/30' :
            session.type === 'fitness' ? 'bg-orange-500/15 text-orange-600 dark:bg-orange-500/20 dark:text-orange-400 border border-orange-500/30' :
            'bg-blue-500/15 text-blue-600 dark:bg-blue-500/20 dark:text-blue-400 border border-blue-500/30'
          ]"
        >
          {{ sessionTypeLabel(session.type) }}
        </span>
        <h4 class="font-bold text-sm text-gray-900 dark:text-white leading-snug">{{ session.title }}</h4>
      </div>
    </div>

    <p v-if="session.description" class="text-xs text-gray-600 dark:text-gray-400 mb-3 line-clamp-2">
      {{ session.description }}
    </p>

    <!-- Lokacija i Vreme -->
    <div class="space-y-1 text-xs text-gray-600 dark:text-gray-400 mb-3 bg-gray-50 dark:bg-gray-900/60 p-2.5 rounded-lg border border-gray-200 dark:border-gray-700/50">
      <div v-if="session.scheduled_at" class="flex items-center gap-1.5 text-indigo-600 dark:text-indigo-300">
      ...
      <div v-if="session.location" class="flex items-center gap-1.5 text-gray-600 dark:text-gray-300">
    ...
    <div class="flex justify-between items-center pt-2 border-t border-gray-100 dark:border-gray-700/50 text-xs">
      ...
      <select 
        ...
        class="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 text-[11px] rounded-lg px-2 py-1 outline-none focus:border-indigo-500 font-medium"
      >
