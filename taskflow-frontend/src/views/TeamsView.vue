  ...
  <div class="max-w-7xl mx-auto p-6">
    <div class="flex justify-between items-center mb-6">
      <div>
        ...
        <p class="text-sm text-gray-400 mt-1">Upravljanje selekcijama akademije i dodeljivanje igrača</p>
      </div>
      ...
      >
        <div>
          <div class="flex justify-between items-start mb-3">
            ...
            <span class="bg-indigo-600/20 text-indigo-400 border border-indigo-500/30 text-xs font-mono font-bold uppercase px-2.5 py-1 rounded-md">
              {{ team.age_group }}
            </span>
          </div>
          ...
          <div class="mt-4">
            ...
            <div v-if="team.members && team.members.length > 0" class="space-y-1.5 max-h-48 overflow-y-auto pr-1">
              <div 
              ...
              >
                ...
                <span class="text-indigo-400 font-mono font-bold">#{{ member.player_profile?.jersey_number || '-' }}</span>
              </div>
            </div>
            <p v-else class="text-xs text-gray-500 italic">Nema dodeljenih igrača u ovoj ekipi.</p>
          </div>
        </div>
      ...
      <div class="bg-gray-800 border border-gray-700 rounded-xl p-6 w-full max-w-md shadow-2xl">
        ...
        <form @submit.prevent="handleCreateTeam" class="space-y-4">
          ...
          <div>
            ...
            <select v-model="newTeam.age_group" class="w-full bg-gray-900 border border-gray-700 rounded p-2.5 text-white outline-none">
          ...
          <div class="flex justify-end space-x-3 pt-4 border-t border-gray-700">
            <button type="button" @click="showCreateModal = false" class="px-4 py-2 text-sm text-gray-400 hover:text-white">Odustani</button>
...
const openAssignModal = (team) => {
  // Ovde ćemo otvoriti selektor za dodeljivanje igrača ekipi
}
</script>
